...
#define MQTT_KEY         "gruppe03"
#define MQTT_TOPIC       "/count"  
#define MQTT_TOPIC2      "/voltage"  
...
Adafruit_MQTT_Publish count_mqtt_publish_voltage = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME MQTT_TOPIC2);
...
void publish_data()
{
  ...
  if((WiFiMulti.run(conn_tout_ms) == WL_CONNECTED))
  {
    ...
    mqtt_connect();
    if (! count_mqtt_publish.publish(payload) || ! count_mqtt_publish_voltage.publish(payload2))
    {
      debug("MQTT failed");
    }
    else
